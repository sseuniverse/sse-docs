<template>
  <div class="relative px-4 sm:px-6 lg:px-8">
    <div
      class="stars w-screen fixed pointer-events-none inset-x-0 -top-[--header-height] bottom-0 opacity-75"
    >
    ...
    <UPageBody>
      <div
        class="h-[96px] w-0.5 bg-gray-200 dark:bg-gray-800 mx-auto rounded-t-full"
      />
      ...
      >
        ...
        <template
          v-if="date.release || isToday(date.day)"
        >
          <div
            ...
            :class="
              index % 2 === 0
                ? 'translate-x-[50%] -ml-2'
                : '-translate-x-[50%] ml-2 flex-row-reverse'
            "
          >
            <div
      ...
      <div
        class="h-[96px] w-0.5 bg-gray-200 dark:bg-gray-800 mx-auto rounded-b-full"
      />
    </UPageBody>
  </div>
</template>
...
const { data: page } = await useAsyncData("releases", () =>
  queryContent("/dev/releases").findOne()
);
if (!page.value) {
  throw createError({
    statusCode: 404,
