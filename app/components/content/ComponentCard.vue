<template>
  <div>
    <div
      v-if="propsToSelect.length"
      class="relative flex border border-gray-200 dark:border-gray-700 rounded-t-md overflow-hidden not-prose"
    >
      <div
    ...
    <div
      ...
      :class="[
        { 'p-4': padding },
        propsToSelect.length ? 'border-t-0' : 'rounded-t-md',
        backgroundClass,
        extraClass,
      ]"
    >
    ...
    type: Array as PropType<
      {
        name: string;
        values: string[];
        restriction: "expected" | "included" | "excluded" | "only";
      }[]
    >,
    default: () => [],
  },
...
let name =
  props.slug ||
  `U${upperFirst(camelCase(route.params.slug[route.params.slug.length - 1]))}`;
...
const generateOptions = (
  key: string,
  schema: { kind: string; schema: []; type: string }
) => {
  let options = [];
  const optionItem = props?.options?.find((item) => item?.name === key) || null;
  const types =
    schema?.type?.split("|")?.map((item) => item.trim()?.replaceAll('"', "")) ||
    [];
  const hasIgnoredTypes = types?.every((item) =>
    ["string", "number", "boolean", "array", "object", "Function"].includes(
      item
    )
  );
  ...
  for (const [key, value] of Object.entries(fullProps.value)) {
    ...
    code += ` ${
      (typeof value === "boolean" &&
        (value !== true || key === "modelValue")) ||
      typeof value === "object" ||
      typeof value === "number"
        ? ":"
        : ""
    }${key === "modelValue" ? "model-value" : kebabCase(key)}${
      typeof value === "boolean" && !!value && key !== "modelValue"
        ? ""
